#!/bin/bash
PATH=/sbin:/bin:/usr/sbin:/usr/bin:/opt/pgpro/1c-14/bin;

#Скрипт для бэкапа и обслуживания баз данных PostgreSQL 1C

#
PGPRO=/opt/pgpro/1c-14/bin
...
Backup()
{
	...
	sudo -u postgres $PGPRO/pg_dump -U postgres "$1" | pigz > "$BACKUPDIR"/"$DATA"-"$1".sql.gz 2>>"$LOGS"/"$DATA".log
        if [[  $? -eq 0 ]]; then
...
Maintance()
{
	if [[ $(date "+%u") == 6 ]]; then
		...
		if sudo -Hiu postgres $PGPRO/vacuumdb --full --analyze --username postgres --dbname "$1" > /dev/null 2>&1 ; then
		...
		if sudo -Hiu postgres $PGPRO/reindexdb --username postgres --dbname "$1" > /dev/null 2>&1 ; then
		...
		if sudo -Hiu postgres $PGPRO/vacuumdb --analyze --username postgres --dbname "$1" > /dev/null 2>&1 ; then
